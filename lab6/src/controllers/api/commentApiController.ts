...
export const listCommentsController = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
...
export const createCommentController = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (!fanfic_id || !text || !user_id) {
        res.status(400).json({ error: 'fanfic_id, text and auth token are required' });
        return;
    }
